...
sv_fit <- function(y,theta,P,estimate){
  ...
  if (estimate){
    ...
    optimized <- nloptr(theta, eval_f = function(x)(sv_loglik(x, y, eta_sim, u_sim, alpha_up_0, alpha_wt_0)), opts = opts, lb=lb, ub=ub, )
  ...
  loglik = cc[1]
  alpha <- cc[2]
  return(loglik, optimized$solution, alpha, theta_se)
